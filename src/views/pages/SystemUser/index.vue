<template>
  <div>
    <vxe-toolbar ref="toolbarRef" custom :refresh="{queryMethod:fetch}">
      <template #buttons>
        <vxe-button v-auth:create @click="router.push('/system_user/operation')"  content="添加" status="primary"
    ...
    <div style="height: calc(100vh - 50px - 0.5em - 100px  - 50px)">
      <vxe-table    ref="tableRef" height="100%" show-overflow :data="listing">
        ...
        <vxe-column  field="account" title="账户"></vxe-column>
        <vxe-column  field="system_role_id" title="角色"></vxe-column>
        ...
        <vxe-column title="操作" width="200" :visible="auth(['delete','update'])">
          <template #default="{row}">
            ...
            <vxe-button v-auth:update status="primary" @click="router.push(`/system_user/operation/${row?.id}`)"
...
const fetch = async () => {
  const {list, total}:any = await request.get('/system_user', {params})
  listing.value = list;
  params.total = total;
}
